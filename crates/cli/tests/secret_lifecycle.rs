...
fn secret_rotate_new_value_changes_what_run_injects() {
    ...
    assert!(sc(
        &repo,
        &["secret", "add", "DB_URL", "--to", "alice", "--value", "v0"]
    )
    .status
    .success());
...
fn escrow_set_and_show_roundtrip() {
    ...
    assert!(String::from_utf8_lossy(&none.stdout)
        .to_lowercase()
        .contains("no escrow"));
    ...
    assert!(
        out.to_lowercase().contains("policy") || out.to_lowercase().contains("not enforce"),
...
fn escrow_is_auto_included_on_add_and_recoverable() {
    ...
    assert!(
        out.contains("DB_URL") && out.contains("2 recipient"),
        "escrow auto-included: {out}"
    );
    ...
    let code = sc(
        ...
        &[
            "run",
            "--identity",
            escrow_id.to_str().unwrap(),
            "--",
            "sh",
            "-c",
            "test \"$DB_URL\" = topsecret",
        ],
    )
    .status
    .code()
    ...
    assert!(
        sc(&repo, &["secret", "rotate", "DB_URL", "--value", "rotated"])
            .status
            .success()
    );
    let list2 = sc(&repo, &["secret", "list"]);
    assert!(
        String::from_utf8_lossy(&list2.stdout).contains("2 recipient"),
        "escrow retained on rotate"
    );
    let code2 = sc(
        &repo,
        &[
