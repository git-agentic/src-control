...
mod tests {
    ...
    fn blob_commit_ref_roundtrip() {
        ...
        // git can read it back.
        let out = git(&dir, &["--git-dir", dir.to_str().unwrap(), "log", "--format=%s", "-1", "main"]);
...
impl GitTarget {
    ...
    /// (directory names compared as if suffixed with `/`). Returns the tree oid.
    pub(crate) fn write_tree(&self, entries: Vec<GitTreeEntry>) -> Result<gix::ObjectId> {
        ...
        // Git requires entries sorted by the trailing-slash-for-trees rule.
        // `gix::objs::Tree` entries implement that ordering via `Ord`.
        tree.entries.sort();
...
Run: `cargo test -p scl-gitio export::tests`
Expected: PASS — both `write_tree_matches_canonical_git_order` (proving canonical order) and `blob_commit_ref_roundtrip`. If the tree-order test fails, the sort rule is wrong: do NOT plain-sort by name; use `gix`'s tree-entry ordering (which appends `/` to tree names) — that is the whole point of this task.
...
```
Note: `import_head` resolves `HEAD`; `export_branch` wrote `refs/heads/main`. Confirm the fresh bare repo's `HEAD` points at `refs/heads/main` (gix `init_bare` defaults `HEAD` to `refs/heads/main` or `master` depending on config). If `import_head` fails to resolve HEAD on the exported repo, set `ref_name` in this test to whatever the bare repo's default `HEAD` names, OR have the test read HEAD via git. Keep the assertion on the reconstructed tree.
...
- **Round-trip HEAD caveat (Task 3 Step 5):** a freshly `init_bare`'d repo's default `HEAD` branch name (`main` vs `master`) may differ from `ref_name`; the test notes how to reconcile so `import_head` (which resolves `HEAD`) finds the exported commit.
