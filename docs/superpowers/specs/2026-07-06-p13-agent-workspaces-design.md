...
three `work/<label>` branches (one per changed workspace) integrable with the
...
- Workspace labels default to `1..N`; `--name feature` yields labels
  `feature-1..feature-N`. Branch per changed workspace: `work/<label>`.
...
2. **Preflight (fail-fast, before any agent runs):**
   ...
   - refuse if any target branch `work/<label>` already exists;
...
6. **Harvest (sequential, under the held lock).** For each workspace:
   ...
   the others. Changed workspaces commit to `work/<label>` (parent = base
...
- **No silent destruction.** Failed agents keep their branch; scanner
  rejections are reported, not swallowed; existing `work/<label>` branches
